...
function App() {
  const [resumeData, setResumeData] = useState({
    ...
    workExperience: [] as { company: string; position: string; startDate: string; endDate: string; description: string }[],
        ...
        <div className="w-1/2 p-8">
          ...
          {/* This form will need to move into a seperate component */}
            <form>
              ...
              <div className="form-group p-4 border-gray-200">
                <label className='font-bold' htmlFor="linkedin">LinkedIn</label>
                <input
                className="m-2 p-2 border-b-2 border-gray-400 focus:border-blue-600 outline-none bg-transparent"
                type="url"
                id="linkedin"
              ...
              <div className="form-group p-4 border-gray-200">
                <label className='font-bold' htmlFor="github">GitHub</label>
                <input
                className="m-2 p-2 border-b-2 border-gray-400 focus:border-blue-600 outline-none bg-transparent"
                type="url"
                id="github"
              ...
              <div className="form-group p-4 border-gray-200">
                <label className='font-bold' htmlFor="phone">Professional Summary</label>
                <textarea 
  ...
  <div className="resume-output p-6 border border-gray-200 bg-white rounded shadow space-y-6">
    ...
    <div className="flex flex-col gap-4 items-center justify-center text-center">
      <h3 className="text-gray-700 text-2xl">{resumeData.name || 'Your Name'}</h3>
      <div className="flex flex-row gap-2 justify-center">
        <p className="text-gray-700">{resumeData.email || 'Your Email'}</p>
        {resumeData.phone && (
          <span className="text-gray-700">|</span>
        )}
        <p className="text-gray-700">{resumeData.phone || 'Your Phone'}</p>
        {resumeData.location && (
          <span className="text-gray-700">|</span>
        )}
        <p className="text-gray-700">{resumeData.location || 'Your Location'}</p>
        {resumeData.linkedin && (
          <span className="text-gray-700">|</span>
        )}
        <p className="text-gray-700">{resumeData.linkedin || 'Your LinkedIn'}</p>
        {resumeData.github && (
          <span className="text-gray-700">|</span>
        )}
        <p className="text-gray-700">{resumeData.github || 'Your GitHub'}</p>
      </div>
    </div>
